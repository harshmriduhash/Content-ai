...
export async function generateContent({
  ...
  return (
    `Generated ${contentType} content based on prompt: "${prompt}"\n\n` +
...
export async function generateImage({
  ...
  return `https://source.unsplash.com/random/${dimensions}x${dimensions}/?${encodeURIComponent(
    query
  )}`;
}
...
export async function editVideo({
  ...
  action,
  options,
}: VideoEditParams): Promise<string> {
  // Simulate API call
  await new Promise((resolve) => setTimeout(resolve, 2000));
